...
<form id="order_review" method="post">
	...
	<table class="store_table">
		...
		<tfoot>
			<tr>
				<td colspan="2"><?php _e('Subtotal', 'cmdeals'); ?></td>
				<td><?php echo $order->get_subtotal_to_display(); ?></td>
			</tr>
			<tr>
				<td colspan="2"><strong><?php _e('Grand Total', 'cmdeals'); ?></strong></td>
				<td><strong><?php echo cmdeals_price($order->order_total); ?></strong></td>
			</tr>
		</tfoot>
		<tbody>
			<?php
				...
				foreach ($order->items as $item) :
					echo '
						<tr>
							<td>'.$item['name'].'</td>
							<td>'.$item['qty'].'</td>
							<td>'.cmdeals_price( $item['cost']*$item['qty'] ).'</td>
						</tr>';
				endforeach; 
			endif;
			?>
	...
	<div id="payment">
		...
		<ul class="payment_methods methods">
			<?php 
				...
				if ($available_gateways) : 
					...
					echo '<p>'.__('Sorry, it seems that there are no available payment methods for your location. Please contact us if you require assistance or wish to make alternate arrangements.', 'cmdeals').'</p>';
		...
		<div class="form-row">
			<?php $cmdeals->nonce_field('pay')?>
			<input type="submit" class="button-alt" name="pay" id="place_order" value="<?php _e('Pay for order', 'cmdeals'); ?>" />
