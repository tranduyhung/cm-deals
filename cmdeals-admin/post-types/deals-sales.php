...
function cmdeals_edit_order_columns($columns){
	...
	$columns["sales_status"] = __("Status", 'cmdeals');
	$columns["order_title"] = __("Order", 'cmdeals');
	$columns["total_cost"] = __("Order Total", 'cmdeals');
	$columns["order_date"] = __("Date", 'cmdeals');
	$columns["order_actions"] = __("Actions", 'cmdeals');
...
function cmdeals_custom_order_columns($column) {
	...
	switch ($column) {
		case "sales_status" :
			...
			echo sprintf( __('<mark class="%s">%s</mark>', 'cmdeals'), sanitize_title($order->status), __($order->status, 'cmdeals') );
		...
		case "order_actions" :
			...
			?><p>
				<?php if (in_array($order->status, array('pending', 'on-hold'))) : ?><a class="button" href="<?php echo wp_nonce_url( admin_url('admin-ajax.php?action=cmdeals-mark-order-processing&order_id=' . $post->ID) ); ?>"><?php _e('Processing', 'cmdeals'); ?></a><?php endif; ?>
				<?php if (in_array($order->status, array('pending', 'on-hold', 'processing'))) : ?><a class="button" href="<?php echo wp_nonce_url( admin_url('admin-ajax.php?action=cmdeals-mark-order-complete&order_id=' . $post->ID) ); ?>"><?php _e('Complete', 'cmdeals'); ?></a><?php endif; ?>
				<a class="button" href="<?php echo admin_url('post.php?post='.$post->ID.'&action=edit'); ?>"><?php _e('View', 'cmdeals'); ?></a>
			</p><?php
