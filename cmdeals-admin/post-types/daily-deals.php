...
function cmdeals_edit_deals_columns($columns){
	...
	$columns["thumb"] = __("Image", 'cmdeals');
	$columns["title"] = __("Name", 'cmdeals');
	$columns["deal-type"] = __("Type", 'cmdeals');
	$columns["deal-categories"] = __("Categories", 'cmdeals');
	$columns["price"] = __("Price", 'cmdeals');
	$columns["featured"] = __("Featured", 'cmdeals');
	$columns["status"] = __("Status", 'cmdeals');
	$columns["date"] = __("Date", 'cmdeals');
...
function cmdeals_custom_deals_columns($column) {
	...
	switch ($column) {
		...
		case "featured" :
			...
			if ($deal->is_featured()) echo '<a href="'.$url.'"><img src="'.$cmdeals->plugin_url().'/cmdeals-assets/images/success.gif" alt="yes" />';
			else echo '<img src="'.$cmdeals->plugin_url().'/cmdeals-assets/images/success-off.gif" alt="no" />';
			echo '</a>';
		break;
		case "status" :
			if ( !$deal->is_expired() ) :
