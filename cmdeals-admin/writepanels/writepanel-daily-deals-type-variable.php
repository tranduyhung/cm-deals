...
function variable_deals_type_options_tab() {
	?>
	<li class="variations_tab show_if_variable"><a href="#variable_deals_options" title="<?php _e('Variations for variable deals are defined here.', 'cmdeals'); ?>"><?php _e('Variations', 'cmdeals'); ?></a></li>
	<?php
}

add_action('cmdeals_deals_write_panel_tabs', 'variable_deals_type_options_tab'); 
...
function variable_deals_type_options() {
	...
	<div id="variable_deals_options" class="panel">
		...
		<?php if (!$variation_attribute_found) : ?>
			<div class="inline updated"><p><?php _e('Before you can start adding variations you must set up and save some variable attributes via the <strong>Attributes</strong> tab.', 'cmdeals'); ?></p></div>
		<?php else : ?>
	
			<p class="bulk_edit"><strong><?php _e('Bulk edit:', 'cmdeals'); ?></strong> <a class="button set set_all_prices" href="#"><?php _e('Prices', 'cmdeals'); ?></a> <a class="button set set_all_sale_prices" href="#"><?php _e('Sale prices', 'cmdeals'); ?></a> <a class="button set set_all_stock" href="#"><?php _e('Stock', 'cmdeals'); ?></a> <a class="button toggle toggle_downloadable" href="#"><?php _e('Downloadable', 'cmdeals'); ?></a> <a class="button toggle toggle_virtual" href="#"><?php _e('Virtual', 'cmdeals'); ?></a> <a class="button toggle toggle_enabled" href="#"><?php _e('Enabled', 'cmdeals'); ?></a> <a class="button set set_all_paths" href="#"><?php _e('File paths', 'cmdeals'); ?></a> <a class="button set set_all_limits" href="#"><?php _e('Download limits', 'cmdeals'); ?></a></p>
	
			<div class="cmdeals_variations">
				<?php
				$args = array(
				...
				if ($variations) foreach ($variations as $variation) : 
					...
					if (!$image) $image = $cmdeals->plugin_url().'/cmdeals-assets/images/placeholder.png';
					?>
					<div class="cmdeals_variation">
						<p>
							<button type="button" class="remove_variation button" rel="<?php echo $variation->ID; ?>"><?php _e('Remove', 'cmdeals'); ?></button>
							<strong>#<?php echo $variation->ID; ?> &mdash; <?php _e('Variation:', 'cmdeals'); ?></strong>
							<?php
								foreach ($attributes as $attribute) :
									...
									echo '<select name="attribute_' . sanitize_title($attribute['name']).'['.$loop.']"><option value="">'.__('Any', 'cmdeals') . ' ' . $cmdeals->attribute_label($attribute['name']).'&hellip;</option>';
						...
						<table cellpadding="0" cellspacing="0" class="cmdeals_variable_attributes">
							<tbody>	
								<tr>
									...
									<td><label><?php _e('Price:', 'cmdeals'); ?></label><input type="text" size="5" name="variable_price[<?php echo $loop; ?>]" value="<?php if (isset($variation_data['price'][0])) echo $variation_data['price'][0]; ?>" /></td>
									
									<td><label><?php _e('Sale Price:', 'cmdeals'); ?></label><input type="text" size="5" name="variable_sale_price[<?php echo $loop; ?>]" value="<?php if (isset($variation_data['_discount_price'][0])) echo $variation_data['_discount_price'][0]; ?>" /></td>
									
								</tr>
								<tr>
								
									<td><label><?php _e('Downloadable', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enable this option if access is given to a downloadable file upon purchase of a deals.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="checkbox" class="checkbox variable_is_downloadable" name="variable_is_downloadable[<?php echo $loop; ?>]" <?php if (isset($variation_data['downloadable'][0])) checked($variation_data['downloadable'][0], 'yes'); ?> /></td>
	
									<td><label><?php _e('Virtual', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enable this option if a deals is not shipped or there is no shipping cost.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="checkbox" class="checkbox" name="variable_is_virtual[<?php echo $loop; ?>]" <?php if (isset($variation_data['virtual'][0])) checked($variation_data['virtual'][0], 'yes'); ?> /></td>
									
									<td><label><?php _e('Enabled', 'cmdeals'); ?></label><input type="checkbox" class="checkbox" name="variable_enabled[<?php echo $loop; ?>]" <?php checked($variation->post_status, 'publish'); ?> /></td>
									
									<td>
										<div class="show_if_variation_downloadable file_path_field">
										<label><?php _e('File path:', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enter a File Path to make this variation a downloadable deals, or leave blank.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="text" size="5" class="file_path" name="variable_file_path[<?php echo $loop; ?>]" value="<?php if (isset($variation_data['file_path'][0])) echo $variation_data['file_path'][0]; ?>" placeholder="<?php _e('File path/URL', 'cmdeals'); ?>" /> <input type="button"  class="upload_file_button button" value="<?php _e('&uarr;', 'cmdeals'); ?>" title="<?php _e('Upload', 'cmdeals'); ?>" />
										</div>
									</td>
									
									<td>
										<div class="show_if_variation_downloadable">
										<label><?php _e('Download Limit:', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Leave blank for unlimited re-downloads.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="text" size="5" name="variable_download_limit[<?php echo $loop; ?>]" value="<?php if (isset($variation_data['download_limit'][0])) echo $variation_data['download_limit'][0]; ?>" placeholder="<?php _e('Unlimited', 'cmdeals'); ?>" />
										</div>
									</td>
			...
			<p class="default_variation">
				<strong><?php _e('Default variation selections:', 'cmdeals'); ?></strong>
				<?php
					...
					foreach ($attributes as $attribute) :
						...
						// Name will be something like attribute_pa_color
						echo '<select name="default_attribute_' . sanitize_title($attribute['name']).'"><option value="">'.__('No default', 'cmdeals') . ' ' . $cmdeals->attribute_label($attribute['name']).'&hellip;</option>';
			...
			<button type="button" class="button button-primary add_variation" <?php disabled($variation_attribute_found, false); ?>><?php _e('Add Variation', 'cmdeals'); ?></button>
			<button type="button" class="button link_all_variations" <?php disabled($variation_attribute_found, false); ?>><?php _e('Link all variations', 'cmdeals'); ?></button>
			
			<p class="description"><?php _e('Add (optional) information for deals variations. If you modify your deals attributes you must save the deals before they will be selectable.', 'cmdeals'); ?></p>
...
add_action('cmdeals_deals_write_panels', 'variable_deals_type_options');
	...
	jQuery(function(){
		...
		<?php if (!$attributes || (is_array($attributes) && sizeof($attributes)==0)) : ?>
			...
			jQuery('button.link_all_variations, button.add_variation').live('click', function(){
				...
				alert('<?php _e('You must add some attributes via the "Deal Data" panel and save before adding a new variation.', 'cmdeals'); ?>');
		...
		jQuery('button.add_variation').live('click', function(){
			...
			jQuery('.cmdeals_variations').block({ message: null, overlayCSS: { background: '#fff url(<?php echo $cmdeals->plugin_url(); ?>/cmdeals-assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
					
			var data = {
				action: 'cmdeals_add_variation',
			...
			jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) {
				...
				var loop = jQuery('.cmdeals_variation').size();
				
				jQuery('.cmdeals_variations').append('<div class="cmdeals_variation">\
					<p>\
						<button type="button" class="remove_variation button" rel="' + variation_id + '"><?php _e('Remove', 'cmdeals'); ?></button>\
						<strong>#' + variation_id + ' &mdash; <?php _e('Variation:', 'cmdeals'); ?></strong>\
						<?php
							if ($attributes) foreach ($attributes as $attribute) :
					...
					<table cellpadding="0" cellspacing="0" class="cmdeals_variable_attributes">\
						<tbody>\
							<tr>\
								<td class="upload_image" rowspan="2"><a href="#" class="upload_image_button" rel="' + variation_id + '"><img src="<?php echo $cmdeals->plugin_url().'/cmdeals-assets/images/placeholder.png' ?>" width="60px" height="60px" /><input type="hidden" name="upload_image_id[' + loop + ']" class="upload_image_id" /><span class="overlay"></span></a></td>\
								\
								<td><label><?php _e('SKU:', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enter a SKU for this variation or leave blank to use the parent deals SKU.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="text" size="5" name="variable_sku[' + loop + ']" placeholder="<?php if ($sku = get_post_meta($post->ID, 'sku', true)) echo $sku; else echo $post->ID; ?>" /></td>\
								\
								<td><label><?php _e('Stock Qty:', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enter a quantity to manage stock for this variation, or leave blank to use the variable deals stock options.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="text" size="5" name="variable_stock[' + loop + ']" /></td>\
								\
								...
								\
								<td><label><?php _e('Price:', 'cmdeals'); ?></label><input type="text" size="5" name="variable_price[' + loop + ']" /></td>\
								\
								<td><label><?php _e('Sale Price:', 'cmdeals'); ?></label><input type="text" size="5" name="variable_sale_price[' + loop + ']" /></td>\
							</tr>\
							<tr>\
								<td><label><?php _e('Downloadable', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enable this option if access is given to a downloadable file upon purchase of a deals.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="checkbox" class="checkbox variable_is_downloadable" name="variable_is_downloadable[' + loop + ']" /></td>\
								\
								<td><label><?php _e('Virtual', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enable this option if a deals is not shipped or there is no shipping cost.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="checkbox" class="checkbox" name="variable_is_virtual[' + loop + ']" /></td>\
								\
								<td><label><?php _e('Enabled', 'cmdeals'); ?></label><input type="checkbox" class="checkbox" name="variable_enabled[' + loop + ']" /></td>\
								\
								<td>\
									<div class="show_if_variation_downloadable file_path_field" style="display:none;"><label><?php _e('File path:', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Enter a File Path to make this variation a downloadable deals, or leave blank.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="text" size="5" class="file_path" name="variable_file_path[' + loop + ']" placeholder="<?php _e('File path/URL', 'cmdeals'); ?>" /> <input type="button"  class="upload_file_button button" value="<?php _e('&uarr;', 'cmdeals'); ?>" title="<?php _e('Upload', 'cmdeals'); ?>" /></div>\
								</td>\
								\
								<td>\
									<div class="show_if_variation_downloadable" style="display:none;"><label><?php _e('Download Limit:', 'cmdeals'); ?> <a class="tips" tip="<?php _e('Leave blank for unlimited re-downloads.', 'cmdeals'); ?>" href="#">[?]</a></label><input type="text" size="5" name="variable_download_limit[' + loop + ']" placeholder="<?php _e('Unlimited', 'cmdeals'); ?>" /></div>\
								</td>\
								<td>&nbsp;</td>\
							</tr>\
						</tbody>\
				...
				jQuery('.cmdeals_variations').unblock();
		...
		jQuery('button.link_all_variations').live('click', function(){
			...
			var answer = confirm('<?php _e('Are you sure you want to link all variations? This will create a new variation for each and every possible combination of variation attributes.', 'cmdeals'); ?>');
			...
			if (answer) {
				...
				var data = {
					action: 'cmdeals_link_all_variations',
					post_id: <?php echo $post->ID; ?>,
				...
				jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) {
					...
					if (response==1) {				
						jQuery('.cmdeals_variations').load( window.location + ' .cmdeals_variations > *' );
					}
		...
		jQuery('button.remove_variation').live('click', function(){
			var answer = confirm('<?php _e('Are you sure you want to remove this variation?', 'cmdeals'); ?>');
			if (answer){
				...
				if (variation>0) {
					...
					jQuery(el).block({ message: null, overlayCSS: { background: '#fff url(<?php echo $cmdeals->plugin_url(); ?>/cmdeals-assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
					
					var data = {
						action: 'cmdeals_remove_variation',
		...
		jQuery('a.set_all_prices').click(function(){
			var value = prompt("<?php _e('Enter a price', 'cmdeals'); ?>");
		...
		jQuery('a.set_all_stock').click(function(){
			var value = prompt("<?php _e('Enter stock quantity', 'cmdeals'); ?>");
		...
		jQuery('a.set_all_paths').click(function(){
			var value = prompt("<?php _e('Enter a file path/URL', 'cmdeals'); ?>");
			jQuery('input[name^="variable_file_path"]').val( value );
			return false;
		});
		
		jQuery('a.set_all_limits').click(function(){
			var value = prompt("<?php _e('Enter a download limit', 'cmdeals'); ?>");
		...
		jQuery('.upload_image_button').live('click', function(){
			...
			if (jQuery(this).is('.remove')) {
				...
				jQuery('img', current_field_wrapper).attr('src', '<?php echo $cmdeals->plugin_url().'/cmdeals-assets/images/placeholder.png'; ?>');
...
add_action('cmdeals_deals_write_panel_js', 'variable_deals_write_panel_js');
...
function variable_deals_type_selector( $types, $deal_type ) {
	$types['variable'] = __('Variable deals', 'cmdeals');
	return $types;
}
		...
		for ($i=0; $i<sizeof($variable_sku); $i++) :
			...
			foreach ($attributes as $attribute) :
				if ( $attribute['is_variation'] ) :
					...
					if ($value) :
						$title[] = $cmdeals->attribute_label($attribute['name']).': '.$value;
					endif;
				endif;
			endforeach;
...
add_action('cmdeals_process_deals_meta_variable', 'process_deals_meta_variable');